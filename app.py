...
async def reset(request: Optional[ResetRequest] = None):
    ...
    # Support both 'task' and 'task_id' field names
    task_id = request.task or request.task_id or "task_easy"
    if task_id not in TASK_IDS:
        raise HTTPException(status_code=400, detail=f"Unknown task_id. Choose from: {list(TASK_IDS)}")
